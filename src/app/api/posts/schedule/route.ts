...
export const POST = async (req: Request) => {
	return withTiming("POST /api/posts/schedule", async () => {
		try {
			const { content, scheduledFor } = await req.json();

			console.debug("Scheduling post:", { content, scheduledFor });
			...
			try {
				...
				console.debug("Validated post data:", newPostData);
			} catch {
			...
			try {
				...
				console.debug("Post scheduled successfully:", scheduledPosts);
				return NextResponse.json({
					success: true,
