...
export const POST = async (req: Request) => {
    try {
        ...
        if (!tokenValidation.isValid) return createTokenValidationResponse(tokenValidation.error || "Token validation failed");
        if (!tokenValidation.user?.id) return NextResponse.json({ success: false, message: "User not found" }, { status: 401 });
        ...
        if (post.userId.toString() !== tokenValidation.user.id) return NextResponse.json({ success: false, message: "Forbidden" }, { status: 403 });
